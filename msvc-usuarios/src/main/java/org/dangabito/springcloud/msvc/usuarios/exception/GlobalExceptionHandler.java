...
import org.dangabito.springcloud.msvc.usuarios.response.ApiError;
...
public class GlobalExceptionHandler {
	...
	public ResponseEntity<ApiError> handleAllException(Exception exception, HttpServletRequest request,
		...
		if (exception instanceof ObjectNotFoundException objectNotfoundException) {
			...
			return this.handleFeignException(feignException, request, response, timeStamp);
		} else {
			logger.info("HANDLEAR GENERAL");
			return this.handleException(exception, request, response, timeStamp);
		}
	}

	private ResponseEntity<ApiError> handleFeignException(FeignException feignException,
		...
		ApiError apiError= new ApiError(httpStatus, request.getRequestURL().toString(), request.getMethod(),
				"No existe el usaurio por el id o error en la comunicaión",
				feignException.getMessage(),timeStamp,null);
